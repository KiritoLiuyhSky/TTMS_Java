...
public class SeatChooseUICus extends PopUITmplTic implements ActionListener {
		...
		Action act = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ticketArray[i][j].getStatus()==0) {
					...
					SellTicketUICus.handler.addTicket(ticketArray[i][j]);
					SellTicketUICus.detail.setText(SellTicketUICus.handler.getInfo());
				} else if (ticketArray[i][j].getStatus()==2) {
					ticketArray[i][j].setStatus(0);
					site.setIcon(siteimgwhite);
					SellTicketUICus.handler.removeTicket(ticketArray[i][j]);
					SellTicketUICus.detail.setText(SellTicketUICus.handler.getInfo());
				}
			}
		};
