...
public class SeatChooseUIEmp extends PopUITmplTic implements ActionListener {
		...
		Action act = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ticketArray[i][j].getStatus()==0) {
					...
					SellTicketUIEmp.handler.addTicket(ticketArray[i][j]);
					SellTicketUIEmp.detail.setText(SellTicketUIEmp.handler.getInfo());
				} else if (ticketArray[i][j].getStatus()==2) {
					ticketArray[i][j].setStatus(0);
					site.setIcon(siteimgwhite);
					SellTicketUIEmp.handler.removeTicket(ticketArray[i][j]);
					SellTicketUIEmp.detail.setText(SellTicketUIEmp.handler.getInfo());
				}
			}
		};
